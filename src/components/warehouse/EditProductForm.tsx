...
export default function EditProductForm({ product, categories, suppliers }: { product: any, categories: any[], suppliers: any[] }) {
    ...
    useEffect(() => {
        if (product) {
            ...
            setBarcodes(product.barcodes?.map((b: any) => b.code) || []);
        }
    }, [product]);
    ...
    const handleSubmit = async (e: React.FormEvent) => {
        ...
        if (result.success) {
            ...
            router.push(`/warehouse/products?${searchParams.toString()}`);
        } else {
    ...
    return (
        <div style={{ maxWidth: '640px', margin: '0 auto', paddingBottom: '4rem', animation: 'fadeIn 0.5s' }}>
            <header style={{ marginBottom: '2rem' }}>
                <h1 style={{ fontSize: '2rem', marginBottom: '0.5rem' }}>Mahsulotni Tahrirlash</h1>
                <p style={{ color: 'var(--text-secondary)' }}>"{product?.name}" ma'lumotlarini o'zgartiring.</p>
            </header>
            ...
            <form onSubmit={handleSubmit} className="card" style={{ display: 'flex', flexDirection: 'column', gap: '1.5rem' }}>
                ...
                <div style={{ display: 'flex', gap: '1rem', marginTop: '0.5rem' }}>
                    ...
                    <button type="submit" disabled={loading} className="btn" style={{ flex: 2 }}>{loading ? "Saqlanmoqda..." : "O'zgarishlarni Saqlash"}</button>
                </div>
            </form>
        </div>
    );
